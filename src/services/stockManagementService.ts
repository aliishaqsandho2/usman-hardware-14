...
class StockManagementService {
  ...
  async validateStockAvailability(productId: number, requestedQuantity: number): Promise<StockValidationResult> {
    try {
      ...
      const response = await fetch(`https://zaidawn.site/wp-json/ims/v1/products/${productId}`, {
        method: 'GET',
  ...
  async deductStock(
    ...
    try {
      ...
      const response = await fetch('https://zaidawn.site/wp-json/ims/v1/inventory/restock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          productId: productId,
