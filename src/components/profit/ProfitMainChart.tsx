...
export default function ProfitMainChart() {
  ...
  const { data: weeklyTrends, isLoading: weeklyLoading } = useQuery({
    ...
    queryFn: () => profitApi.getWeeklyTrends(12), // Last 12 weeks for daily view
  });
  ...
  const isLoading = weeklyLoading || monthlyLoading || ytdLoading;

  // Transform data for charts
  const dailyChartData = weeklyTrends?.map(trend => ({
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Comprehensive profit analysis across different time periods
            </p>
          </div>
        </div>
      </CardHeader>
            ...
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={monthlyChartData}>
                ...
                <Bar 
                  ...
                  radius={[4, 4, 0, 0]}
                />
                <Line type="monotone" dataKey="margin" stroke="#F59E0B" strokeWidth={3} />
              </BarChart>
            </ResponsiveContainer>
          </TabsContent>
